...
File_PadName = "A4-2"
Sub_folder = '12th_SET_const_0.75V_3V_sweep'
print(Sub_folder)
...
for i in range(50):
    ...
    # PTE measurement
    for T_SET in np.linspace(1e-9,50e-9,50):
        tt = '{:.1e}'.format(T_SET)
        print(tt)
        V_SET = 0.75
        ...
        record_resistance(Record_file, V_SET, R_read, f'after_SET_{tt}s')
        ...
        # RESET
        HRS = 1e1
        for V_RESET in np.linspace(V_RESET_0,2.8,5):
            if HRS < HRS_lim:
